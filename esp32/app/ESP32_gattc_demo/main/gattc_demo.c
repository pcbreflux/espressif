...
static void esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_READ_CHAR_EVT: {
        ...
        if (p_data->read.status==0) {
			if (char_id->uuid.len == ESP_UUID_LEN_16) {
				ESP_LOGI(GATTC_TAG,  "Char UUID16: %x", char_id->uuid.uuid.uuid16);
			} else if (char_id->uuid.len == ESP_UUID_LEN_32) {
				ESP_LOGI(GATTC_TAG,  "Char UUID32: %x", char_id->uuid.uuid.uuid32);
			} else if (char_id->uuid.len == ESP_UUID_LEN_128) {
				ESP_LOGI(GATTC_TAG,  "Char UUID128: %x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x", char_id->uuid.uuid.uuid128[0],
    ...
    case ESP_GATTC_WRITE_CHAR_EVT: {
        ...
        ESP_LOGI(GATTC_TAG,  "WRITE CHAR: write.status = %x inst_id = %x", p_data->write.status, char_id->inst_id);
        if (p_data->write.status==0) {
			if (char_id->uuid.len == ESP_UUID_LEN_16) {
				...
				ESP_LOGE(GATTC_TAG,  "Char UNKNOWN LEN %d", char_id->uuid.len);
			}
			if (descr_id->uuid.len == ESP_UUID_LEN_16) {
				ESP_LOGI(GATTC_TAG,  "Decr UUID16: %x", descr_id->uuid.uuid.uuid16);
			} else if (descr_id->uuid.len == ESP_UUID_LEN_32) {
				ESP_LOGI(GATTC_TAG,  "Decr UUID32: %x", descr_id->uuid.uuid.uuid32);
			} else if (descr_id->uuid.len == ESP_UUID_LEN_128) {
				ESP_LOGI(GATTC_TAG,  "Decr UUID128: %x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x", descr_id->uuid.uuid.uuid128[0],
				...
				ESP_LOGE(GATTC_TAG,  "Decr UNKNOWN LEN %d", descr_id->uuid.len);
			}
        }
        break;
    }
    ...
    case ESP_GATTC_WRITE_DESCR_EVT: {
        ...
        ESP_LOGI(GATTC_TAG,  "WRITE DESCR: write.status = %x inst_id = %x open.gatt_if = %x", p_data->write.status, char_id->inst_id,gattc_if);
        if (p_data->write.status==0) {
			if (char_id->uuid.len == ESP_UUID_LEN_16) {
				ESP_LOGI(GATTC_TAG,  "Char UUID16: %x", char_id->uuid.uuid.uuid16);
			} else if (char_id->uuid.len == ESP_UUID_LEN_32) {
			...
			if (srvc_id->id.uuid.len == ESP_UUID_LEN_16) {
				ESP_LOGI(GATTC_TAG,  "SRVC UUID16: %x", srvc_id->id.uuid.uuid.uuid16);
			} else if (srvc_id->id.uuid.len == ESP_UUID_LEN_32) {
				ESP_LOGI(GATTC_TAG,  "SRVC UUID32: %x", srvc_id->id.uuid.uuid.uuid32);
			} else if (srvc_id->id.uuid.len == ESP_UUID_LEN_128) {
				ESP_LOGI(GATTC_TAG,  "SRVC UUID128: %x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x", srvc_id->id.uuid.uuid.uuid128[0],
						 srvc_id->id.uuid.uuid.uuid128[1], srvc_id->id.uuid.uuid.uuid128[2], srvc_id->id.uuid.uuid.uuid128[3],
						 srvc_id->id.uuid.uuid.uuid128[4], srvc_id->id.uuid.uuid.uuid128[5], srvc_id->id.uuid.uuid.uuid128[6],
						 srvc_id->id.uuid.uuid.uuid128[7], srvc_id->id.uuid.uuid.uuid128[8], srvc_id->id.uuid.uuid.uuid128[9],
						 srvc_id->id.uuid.uuid.uuid128[10], srvc_id->id.uuid.uuid.uuid128[11], srvc_id->id.uuid.uuid.uuid128[12],
				...
				ESP_LOGE(GATTC_TAG,  "SRVC UNKNOWN LEN %d", srvc_id->id.uuid.len);
			}
    ...
    case ESP_GATTC_NOTIFY_EVT: {
        ...
        ESP_LOGI(GATTC_TAG,  "NOTIFY: notify.is_notify = %x inst_id = %x", p_data->notify.is_notify, char_id->inst_id);
    ...
    case ESP_GATTC_GET_CHAR_EVT: {
        ...
        conn_id = p_data->open.conn_id;
        ESP_LOGI(GATTC_TAG,  "GET CHAR: open.conn_id = %x search_res.conn_id = %x  get_char.conn_id = %x", conn_id,p_data->search_res.conn_id,p_data->get_char.conn_id);
        ESP_LOGI(GATTC_TAG,  "GET CHAR: get_char.char_prop = %x get_char.status = %x inst_id = %x open.gatt_if = %x", p_data->get_char.char_prop, p_data->get_char.status, char_id->inst_id,gattc_if);
        ESP_LOGI(GATTC_TAG,  "remote_bda %x,%x,%x,%x,%x,%x:",p_data->open.remote_bda[0],
        		p_data->open.remote_bda[1],p_data->open.remote_bda[2],
				p_data->open.remote_bda[3],p_data->open.remote_bda[4],
				p_data->open.remote_bda[5]);
        if (p_data->get_char.status==0) {
			if (char_id->uuid.len == ESP_UUID_LEN_16) {
				ESP_LOGI(GATTC_TAG,  "UUID16: %x", char_id->uuid.uuid.uuid16);
			} else if (char_id->uuid.len == ESP_UUID_LEN_32) {
				ESP_LOGI(GATTC_TAG,  "UUID32: %x", char_id->uuid.uuid.uuid32);
			} else if (char_id->uuid.len == ESP_UUID_LEN_128) {
				ESP_LOGI(GATTC_TAG,  "UUID128: %x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x", char_id->uuid.uuid.uuid128[0],
						 char_id->uuid.uuid.uuid128[1], char_id->uuid.uuid.uuid128[2], char_id->uuid.uuid.uuid128[3],
						 char_id->uuid.uuid.uuid128[4], char_id->uuid.uuid.uuid128[5], char_id->uuid.uuid.uuid128[6],
						 char_id->uuid.uuid.uuid128[7], char_id->uuid.uuid.uuid128[8], char_id->uuid.uuid.uuid128[9],
						 char_id->uuid.uuid.uuid128[10], char_id->uuid.uuid.uuid128[11], char_id->uuid.uuid.uuid128[12],
				...
				ESP_LOGE(GATTC_TAG,  "UNKNOWN LEN %d", char_id->uuid.len);
			}
        }
        break;
    }
    case ESP_GATTC_GET_DESCR_EVT: {
        ...
        conn_id = p_data->open.conn_id;
        ESP_LOGI(GATTC_TAG,  "GET DESCR: open.conn_id = %x search_res.conn_id = %x  get_descr.conn_id = %x", conn_id,p_data->search_res.conn_id,p_data->get_descr.conn_id);
        ESP_LOGI(GATTC_TAG,  "GET DESCR: get_descr.status = %x inst_id = %x open.gatt_if = %x", p_data->get_descr.status, char_id->inst_id,gattc_if);
        uint8_t value[2];
        value[0]=0x01;
        value[1]=0x00;
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        ESP_LOGI(GATTC_TAG,  "NOTIFY_EVT: open.conn_id = %x ", p_data->open.conn_id);
